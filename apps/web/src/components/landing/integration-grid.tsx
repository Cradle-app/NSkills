    ...
    <div className="relative overflow-hidden group">
      ...
      >
        ...
        {[...items, ...items, ...items].map((item, index) => (
          <div
            key={index}
            className="flex flex-col items-center justify-center px-8 py-6 rounded-2xl border border-transparent hover:border-[#d97a4a]/30 hover:bg-[#d97a4a]/5 transition-all duration-300 cursor-default group/item flex-shrink-0"
            style={{ minWidth: '120px' }}
          >
            <div className="w-12 h-12 relative mb-3 opacity-50 group-hover/item:opacity-100 transition-all duration-300 group-hover/item:scale-110 grayscale group-hover/item:grayscale-0">
              <Image
                src={item.icon}
                alt={item.name}
                fill
            ...
            <span className="text-xs font-medium text-forge-text-secondary group-hover/item:text-white transition-colors duration-300 whitespace-nowrap">
              {item.name}
            </span>
          </div>
        ))}
  ...
  return (
    <section ref={sectionRef} className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 mb-12">
        <h2
          ref={titleRef}
          className="text-center text-lg md:text-xl font-display font-semibold text-forge-text-secondary uppercase tracking-[0.2em] mb-4"
        >
