...
const BnbMarketplaceContractSchema = z.object({
    contractAddress: z
        .string()
        .regex(/^0x[0-9a-fA-F]{40}$/)
        .default('0x1E15115269D39e6F7D89a73331D7A0aC99a9Fb61'),
});

export class BnbMarketplaceContractPlugin extends BasePlugin<z.infer<typeof BnbMarketplaceContractSchema>> {
    readonly metadata: PluginMetadata = {
        id: 'bnb-marketplace-contract',
        name: 'BNB Marketplace Contract',
        version: '0.1.0',
    ...
    readonly configSchema =
        BnbMarketplaceContractSchema as unknown as z.ZodType<z.infer<typeof BnbMarketplaceContractSchema>>;

    readonly componentPath = 'packages/components/bnb-marketplace';
    readonly componentPackage = '@cradle/bnb-marketplace';

    readonly componentPathMappings: Record<string, PathCategory> = {
        'src/MarketplaceInteractionPanel.tsx': 'frontend-components',
    ...
    getDefaultConfig(): Partial<z.infer<typeof BnbMarketplaceContractSchema>> {
        return {
    ...
    async generate(node: BlueprintNode, context: ExecutionContext): Promise<CodegenOutput> {
        ...
        this.addFile(output, 'contracts/marketplace/SimpleMarketplace.sol', MARKETPLACE_SOL_SOURCE, 'contract-source');

        this.addFile(output, 'contracts/marketplace/marketplace-abi.json', MARKETPLACE_ABI_JSON, 'contract-source');

        this.addFile(
            output,
            'src/components/MarketplaceInteractionPanel.tsx',
            generateMarketplacePanelWrapper(config.contractAddress),
            'frontend-components',
        );

        context.logger.info('Configured BNB Marketplace contract', {
            nodeId: node.id,
...
const MARKETPLACE_ABI_JSON = JSON.stringify(MARKETPLACE_ABI, null, 2);

const MARKETPLACE_SOL_SOURCE = `// SPDX-License-Identifier: MIT
...
function generateMarketplacePanelWrapper(contractAddress: string): string {
    return `
'use client';
import { MarketplaceInteractionPanel } from '@cradle/bnb-marketplace';

export function MarketplaceInteractionPanelWrapper() {
  return <MarketplaceInteractionPanel contractAddress="${contractAddress}" />;
}
`.trim();
}
